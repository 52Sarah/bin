...
. ~/.__login.debug.sh ".bashrc_promo" || __echo() { :; }
__echo "[.bashrc_promo] starting"
...
if [[ -n "$(jenv version 2> /dev/null)" ]]; then
    ...
    __echo "[.bashrc_promo] used jenv to set JAVA_HOME"
else
    export JAVA_HOME=$(/usr/libexec/java_home -v 1.8)
    export PATH="$JAVA_HOME/bin:$PATH"
    __echo "[.bashrc_promo] used /usr/libexec to set JAVA_HOME"
fi
__echo "[.bashrc_promo] JAVA_HOME=$JAVA_HOME"
...
## __echo "[.bashrc_promo] Node version: $(nvm current)"
...
alias promo.campaign.cd='cd "$CAMPAIGN_JAVA"'
alias promo.db.properties='iecho_and_eval "open "$PROMO_DB_PROPERTIES""'
alias promo.log.tail='iecho_and_eval "tail -n 100 -f "$PROMO_LOG""'


__echo "[.bashrc_promo] finished"
